...
class BytesReader extends _ReadWriteBase {
  ...
  Uint8List readVarSlice() {
    ...
    if (length > BigInt.from(bytes.lengthInBytes - offset)) {
      throw OutOfData(offset, length.toInt(), bytes.lengthInBytes);
    }
  ...
  List<Uint8List> readVector() {
    ...
    // Even an empty item needs one length byte. Check before allocating a list
    // or narrowing an untrusted uint64 to a signed native integer.
    if (count > BigInt.from(bytes.lengthInBytes - offset)) {
      throw OutOfData(offset, count.toInt(), bytes.lengthInBytes);
    }
    return List<Uint8List>.generate(count.toInt(), (i) => readVarSlice());
  }

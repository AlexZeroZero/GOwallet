...
public class StorageCipherFactory {
    ...
    public StorageCipher getSavedStorageCipher(Context context) throws Exception {
        // Migration may only use existing keys; never generate replacements on a read.
        String aesEntry = savedStorageAlgorithm == StorageCipherAlgorithm.AES_GCM_NoPadding
                ? "VGhpcyBpcyB0aGUga2V5IGZvcihBIHNlY3XyZZBzdG9yYWdlIEFFUyBLZXkK"
                : "VGhpcyBpcyB0aGUga2V5IGZvciBhIHNlY3VyZSBzdG9yYWdlIEFFUyBLZXkK";
        if (!context.getSharedPreferences("FlutterSecureKeyStorage", Context.MODE_PRIVATE).contains(aesEntry)) {
            throw new java.security.GeneralSecurityException("Legacy wrapped key is missing");
        }
        String alias = context.getPackageName() + ".FlutterSecureStoragePluginKey"
                + (savedKeyAlgorithm == KeyCipherAlgorithm.RSA_ECB_OAEPwithSHA_256andMGF1Padding ? "OAEP" : "");
        java.security.KeyStore keyStore = java.security.KeyStore.getInstance("AndroidKeyStore");
        keyStore.load(null);
        if (keyStore.getKey(alias, null) == null) {
            throw new java.security.GeneralSecurityException("Legacy wrapping key is unavailable");
        }

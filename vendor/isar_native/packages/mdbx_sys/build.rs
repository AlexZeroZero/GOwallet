...
// GOwallet: use the reviewed, vendored source instead of resolving a mutable tag.
fn copy_source_tree(source: &Path, target: &Path) {
    fs::create_dir_all(target).expect("create native source copy failed");
    for entry in fs::read_dir(source).expect("read vendored native source failed") {
        let entry = entry.expect("read source entry failed");
        let destination = target.join(entry.file_name());
        if entry.file_type().expect("source file type failed").is_dir() {
            copy_source_tree(&entry.path(), &destination);
        } else {
            fs::copy(entry.path(), destination).expect("copy native source failed");
        }
    }
}

fn ensure_libmdbx(out_dir: &Path) -> PathBuf {
    let source = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../libmdbx");
    println!("cargo:rerun-if-changed={}", source.display());
    let destination = out_dir.join("libmdbx");
...
fn main() {
    ...
    if target_os == "android" {
        ...
        // Protect the C database engine without changing its format or features.
        cc_builder.flag("-fstack-protector-strong");
        cc_builder.define("_FORTIFY_SOURCE", "2");
    }

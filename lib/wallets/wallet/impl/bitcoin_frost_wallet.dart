...
class BitcoinFrostWallet<T extends FrostCurrency> extends Wallet<T>
  ...
  Future<void> recover({
    ...
    final recoveredKeys = serializedKeys!;
    ...
    try {
      await refreshMutex.protect(() async {
        ...
        receiveFutures = [
          _checkGapsLinearly(recoveredKeys, receiveChain, secure: true),
        ];
        final List<Future<({int index, List<Address> addresses})>>
        changeFutures = [
          _checkGapsLinearly(recoveredKeys, changeChain, secure: true),
        ];
        ...
        await _legacyInsecureScan(recoveredKeys);
      });

...
import '../gowallet/bounded_fee.dart';
...
class ElectrumXClient {
  ...
  ElectrumXClient({
    ...
    _torService = torService ??
        (AppConfig.hasFeature(AppFeature.tor) ? TorService.sharedInstance : null);
    _host = host;
    _port = port;
    _useSSL = useSSL;
  ...
  Future<Decimal> estimateFee({String? requestID, required int blocks}) async {
    try {
      ...
      try {
        if (response == null ||
            response == -1 ||
            parseBoundedElectrumFee(response) == Decimal.parse("-1")) {
        ...
        final serverRate = parseBoundedElectrumFee(response);

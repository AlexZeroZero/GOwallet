...
enum WalletAction { receive, send, other }
...
class WalletNavigationBarItemData {
  ...
  WalletAction get effectiveAction =>
      action ??
      switch (label) {
        "Receive" => WalletAction.receive,
        "Send" => WalletAction.send,
        _ => WalletAction.other,
      };
    ...
    return GestureDetector(
      ...
      child: RoundedContainer(
        ...
        child: AnimatedOpacity(
          opacity: data.isMore || !ref.watch(walletNavBarMore.state).state
              ? 1
              : 0.2,
                  ...
                  data.overrideText ??
                      Text(
                        ...
                        style: STextStyles.buttonSmall(context).copyWith(
                          color: Theme.of(
                            context,
                          ).extension<StackColors>()!.bottomNavText,
                        ),
                      ),
                ],
              ),
...
class WalletNavigationBarMoreItem extends ConsumerWidget {
  const WalletNavigationBarMoreItem({super.key, required this.data});

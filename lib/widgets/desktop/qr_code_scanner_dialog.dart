...
class _QrCodeScannerDialogState extends State<QrCodeScannerDialog> {
  final CameraLinux? _cameraLinuxPlugin = Platform.isLinux
      ? CameraLinux()
      : null;
  final CameraWindows? _cameraWindowsPlugin = Platform.isWindows
      ? CameraWindows()
      : null;
  ...
  Future<bool> _initializeCamera() async {
    try {
      if (Platform.isLinux && _cameraLinuxPlugin != null) {
        ...
        final List<CameraDescription> cameras = await _cameraWindowsPlugin
            .availableCameras();
        if (cameras.isEmpty) {
        ...
        Logging.instance.d("Windows Camera initialized with ID: $_cameraId");
        ...
        Logging.instance.d("macOS Camera initialized with ID: $_macOSDeviceId");
      }
  ...
  Future<void> _stopCamera() async {
    ...
    try {
      ...
      Logging.instance.e("Failed to stop camera", error: e, stackTrace: s);
    }
  }
    ...
    while (_isScanning) {
      try {
        ...
        if (Platform.isLinux && _cameraLinuxPlugin != null) {
          ...
          final XFile xfile = await _cameraWindowsPlugin!.takePicture(
            _cameraId,
          );
              ...
              child: Row(
                children: [
                  ...
                  SecondaryButton(
                    ...
                    label: goTr(context, "Select file"),
                    width: 200,
                    onPressed: () async {
                      ...
                      if (context.mounted) {
                        if (result == null ||
                          ...
                          await showFloatingFlushBar(
                            ...
                            message: goTr(context, "No file selected"),
                        ...
                        if (filePath == null) {
                          await showFloatingFlushBar(
                            type: FlushBarType.info,
                            message: goTr(context, "Error selecting file."),
                            iconAsset: Assets.svg.file,
                            context: context,
                          );
                          return;
                        ...
                        try {
                          ...
                          if (image == null) {
                            await showFloatingFlushBar(
                              type: FlushBarType.info,
                              message: goTr(context, "Failed to decode image."),
                              iconAsset: Assets.svg.file,
                              context: context,
                            );
                            return;

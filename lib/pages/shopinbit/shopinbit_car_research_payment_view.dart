...
class _ShopInBitCarResearchPaymentViewState
  ...
  void _confirmPayment() {
    ...
    unawaited(
      showFloatingFlushBar(
        ...
        message: goTr(
          context,
          "No compatible wallet for {0}. Pay externally, then tap CHECK FOR PAYMENT.",
          [method],
        ),
        context: context,
      ),
          ...
          unawaited(
            showFloatingFlushBar(
              ...
              message:
                  ...
                  goTr(context, "Failed to fetch invoice status"),
              context: context,
            ),
          );
        }
          ...
          if (mounted) {
            ...
            await showDialog<void>(
              ...
              builder: (ctx) => StackDialog(
                title: goTr(context, "Request Failed"),
                message: goTr(
                  context,
                  "Payment was confirmed but we couldn't submit your car research request. You can retry from My Requests.\n\nError: {0}",
                  [reqResp.exception?.message ?? 'Unknown error'],
                ),
                leftButton: SecondaryButton(
                  label: goTr(context, "Retry Now"),
                  onPressed: () {
                ...
                rightButton: PrimaryButton(
                  label: goTr(context, "My Requests"),
                  onPressed: () {
                    Navigator.of(ctx).pop();
                    _popToTickets();
                  },
        ...
        if (mounted) {
          ...
          unawaited(
            showFloatingFlushBar(
              type: FlushBarType.warning,
              message:
                  logResp.exception?.message ??
                  goTr(context, "Failed to log payment"),
              context: context,
            ),
          );
        }
        ...
        if (mounted) {
          ...
          unawaited(
            showFloatingFlushBar(
              type: FlushBarType.warning,
              message:
                  reqResp.exception?.message ?? goTr(context, "Retry failed"),
              context: context,
            ),
          );
        }
  ...
  void _copyAddress(BuildContext context) {
    ...
    unawaited(
      showFloatingFlushBar(
        type: FlushBarType.info,
        message: goTr(context, "Copied to clipboard"),
  ...
  Widget build(BuildContext context) {
    ...
    final content = Column(
      ...
      children: [
        Text(
          goTr(context, "Car research payment"),
          style: isDesktop
        ...
        if (_currentAddress.isNotEmpty)
          ...
          Center(
            child: Padding(
              ...
              child: Text(
                goTr(context, "No payment address available"),
                style: isDesktop
                    ? STextStyles.desktopTextSmall(context)
                    : STextStyles.itemSubtitle(context),
              ),
              ...
              child: Column(
                children: [
                  Row(
                    children: [
                      Text(
                        goTr(context, "{0} address", [
                          _methods[_selectedMethod],
                        ]),
                        style: isDesktop
                            ? STextStyles.desktopTextExtraExtraSmall(context)
                            : STextStyles.itemSubtitle12(context),
        ...
        PrimaryButton(
          label: _flowState == _PaymentFlowState.polling
              ? goTr(context, "Checking...")
              : (_flowState == _PaymentFlowState.loggingPayment ||
                    _flowState == _PaymentFlowState.creatingRequest)
              ? goTr(context, "Processing...")
              : (hasWallets
                    ? goTr(context, "PAY NOW")
                    : goTr(context, "CHECK FOR PAYMENT")),

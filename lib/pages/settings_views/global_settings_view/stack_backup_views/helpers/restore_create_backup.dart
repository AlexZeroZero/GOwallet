import '../../../../../gowallet/go_backup_input.dart';
...
bool validateFail(
  ...
  for (final e in [
    ...
    [passphrase.trim().runes.length < 12,
      ...
      if (context.mounted) {
        unawaited(
          showFloatingFlushBar(
            ...
            message: goTr(context, e[1] as String),
            context: context,
          ),
        );
      }
...
abstract class SWB {
  ...
  static Future<String?> decryptStackWalletStringWithPassphrase(
    ...
    try {
      if (data.encryptedText.length > goMaxBackupBytes) return null;
      ...
      // Do not log decoder exceptions: they can include backup contents.
      return null;
    }
  }
  ...
  static Future<Map<String, dynamic>> createStackWalletJSON({
    ...
    final Map<String, dynamic> backupJson = {'goCustomCoins': CustomCoinRegistry.instance.exportProfiles()};
  ...
  static Future<bool?> restoreStackWalletJSON(
    ...
    final profiles = CustomCoinDefinition.decodeProfiles(validJSON['goCustomCoins']);
    // Validate the complete set before restoring nodes or resolving wallet IDs.
    CustomCoinRegistry.mergeProfiles(
      CustomCoinRegistry.instance.coins.map((c) => c.definition).toList(), profiles);
    ...
    await CustomCoinRegistry.instance.importProfiles(validJSON['goCustomCoins']);
      ...
      for (final node in nodes) {
        ...
        await nodeService.save(
          NodeModel(
            ...
            id: id,
            useSSL: node['useSSL'] != false && node['useSSL'] != 'false',
            ...
            clearnetEnabled: (node['clearEnabled'] ?? node['clearnetEnabled'] ?? node['plainEnabled']) as bool? ?? true,
            isPrimary:

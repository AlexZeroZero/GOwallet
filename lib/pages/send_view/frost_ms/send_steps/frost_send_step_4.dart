    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          if (kDebugMode)
            DetailItem(
              title: goTr(context, "Tx hex (debug mode only)"),
          ...
          Text(
            goTr(context, "Send {0}", [cryptoCurrency.ticker]),
          ...
          const SizedBox(height: 12),
          DetailItem(
            title: goTr(context, "Transaction fee"),
            detail: ref
                .watch(pAmountFormatter(cryptoCurrency))
          ...
          const SizedBox(height: 12),
          DetailItem(
            title: goTr(context, "Note"),
            detail: ref.watch(pFrostTxData)!.note ?? "",
          ),
          const SizedBox(height: 12),
          DetailItem(title: goTr(context, "Signers"), detail: signers),
          const SizedBox(height: 12),
          if (!Util.isDesktop) const Spacer(),
          const SizedBox(height: 12),
          PrimaryButton(
            label: goTr(context, "Approve transaction"),
            onPressed: () async {
              if (_broadcastLock) {
                return;
              }
              ...
              try {
                ...
                final txData = await showLoading(
                  ...
                  context: context,
                  message: goTr(context, "Broadcasting transaction to network"),
                ...
                if (context.mounted) {
                  return await showDialog<void>(
                    ...
                    builder: (_) => StackOkDialog(
                      title: goTr(context, "Broadcast error"),
                      message: e.toString(),
...
class _Recipient extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        DetailItem(title: goTr(context, "Address"), detail: address),

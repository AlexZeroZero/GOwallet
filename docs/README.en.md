...
[Download v1.0.1](https://github.com/AlexZeroZero/GOwallet/releases/tag/v1.0.1) · [Build](BUILD.md) · [1.0.1 fixes and verification](VERIFICATION-1.0.1.md) · [Security review](SECURITY-REVIEW.md) · [Electrum](ELECTRUM.md)
...
Android package: `org.gowallet.pow`, version `1.0.1+10`, minimum API 24, ARM64/x86_64. The existing GOwallet signing identity is retained for in-place upgrades. Back up first; do not uninstall to upgrade. No iOS or desktop binary is released here.
...
Version 1.0.1 removes Android secure-storage fallback on initialization failure, verifies legacy migration before cleanup, and hardens the unchanged SQLite 3.46.1 source build. All 98 Dart regression tests and 17 Android storage tests passed. An in-place signed upgrade retained the test PIN, custom BTC wallet and SCASH address. ClamAV found 0 infected files. [Full evidence and remaining findings](VERIFICATION-1.0.1.md): MobSF remains **59/100** with **1 high / 7 warnings** in its scorecard; unique native rule counts fell from 10 high / 11 warnings to 7 high / 8 warnings. Isar hardening remains open; no independent audit, physical-device/TEE test or real-fund broadcast is claimed.
